                ...
                <table class="table table-bordered table-striped table-hover" id="">
                    ...
                    @if(!empty($authorities))
                        @foreach($authorities as $topItem)
                                    ...
                                    @if(!empty($operateList))
                                        @foreach($operateList as $operateItem)
                                            ...
                                            <tr>
                                                ...
                                                @endif
                                                @if($operateSecond == 0)
                                                    <td rowspan="{{ $operateLength }}"><input type="checkbox" name="auth_checked[]" value="{{ $secondModel->ts_action }}" @if($secondModel->is_checked)checked="checked"@endif/>&nbsp;&nbsp;{{ $secondModel->name }}({{ $secondModel->ts_action }})</td>
                                                @endif
                                                <td><input type="checkbox" name="auth_checked[]" value="{{ $operateModel->ts_action }}" @if($operateModel->is_checked)checked="checked"@endif/>&nbsp;&nbsp;{{ $operateModel->name }}({{ $operateModel->ts_action }})</td>
                                                <?php $operateSecond++; ?>
                                            </tr>
                                        @endforeach
                                    @else
                            ...
                            @else
                                <tr>
                                    <td><input type="checkbox" name="auth_checked[]" value="{{ $topModel->ts_action }}" @if($topModel->is_checked)checked="checked"@endif/>&nbsp;&nbsp;{{ $topModel->name }}({{ $topModel->ts_action }})</td>
                                    <td rowspan="1"></td>
                                    <td rowspan="1"></td>
                                </tr>
                            @endif
